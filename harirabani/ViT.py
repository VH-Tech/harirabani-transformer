...
class PHMLinearLayer(nn.Module):
    def __init__(self, input_size, output_size):
        super(PHMLinearLayer, self).__init__()
...
class PHMFeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.LayerNorm(dim),
            PHMLinearLayer(dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            PHMLinearLayer(hidden_dim, dim),
            nn.Dropout(dropout)
        )
...
class PHMTransformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout),
                PHMFeedForward(dim, mlp_dim, dropout = dropout)
            ]))
...
class PHMViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.transformer = PHMTransformer(dim, depth, heads, dim_head, mlp_dim, dropout)
